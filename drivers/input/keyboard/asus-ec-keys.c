...
static void asusec_input_handler_deregister(void *priv)
{
...
static int asusec_keys_probe(struct platform_device *pdev)
{
	...
	ret = devm_add_action_or_reset(&pdev->dev, asusec_input_handler_deregister,
				       priv);
	if (ret)
		return ret;
...
static const struct of_device_id asusec_keys_ids[] = {
	{ .compatible = "asus,ec-keys" },
	{ }
};
MODULE_DEVICE_TABLE(of, asusec_keys_ids);

static struct platform_driver asusec_keys_driver = {
	.driver.name = "asusec-keys",
	.driver.of_match_table = of_match_ptr(asusec_keys_ids),
