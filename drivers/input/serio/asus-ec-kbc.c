...
static void asusec_serio_remove(void *data)
{
	serio_unregister_port(data);
}

static int asusec_register_serio(struct platform_device *pdev, int idx,
				 const char *name, int cmd)
{
	...
	return devm_add_action_or_reset(&pdev->dev, asusec_serio_remove, port);
}
...
static int asusec_kbc_probe(struct platform_device *pdev)
{
	...
	ret = asusec_register_serio(pdev, 0, "Keyboard", 0);
	if (ret < 0)
		return ret;

	ret = asusec_register_serio(pdev, 1, "Touchpad", I8042_CMD_AUX_SEND);
	if (ret < 0)
		return ret;
...
static const struct of_device_id asusec_kbc_ids[] = {
	{ .compatible = "asus,ec-kbc" },
	{ }
};
MODULE_DEVICE_TABLE(of, asusec_kbc_ids);

static struct platform_driver asusec_kbc_driver = {
	.driver.name = "asusec-kbc",
	.driver.of_match_table = of_match_ptr(asusec_kbc_ids),
